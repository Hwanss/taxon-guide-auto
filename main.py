...
print(f"🚀 [TaxonGuru] 하이브리드 딥다이브 공장 가동: {TARGET_SPECIES}")
...
다음 5단 구조로 작성해줘 (HTML 형식으로 작성하되 <body> 태그 없이 내용만):
1. Hook: 흥미를 끄는 도발적 질문
2. Scientific Backbone: 분류 체계 표
3. Deep Anatomy: 해부학적 특징
4. Evolutionary Context: 진화적 맥락
5. Verdict & Trivia: 재밌는 썰과 요약

문체: 인기 과학 유튜버처럼 찰진 비유와 유머러스한 톤을 섞어줘.
"""
...
if wiki_image_url:
    image_html = f'<figure><img src="{wiki_image_url}" alt="{TARGET_SPECIES} 학술 자료"><figcaption>[학술 자료] {TARGET_SPECIES}의 실제 모습 및 골격도</figcaption></figure>'
    # Deep Anatomy 섹션 쯤에 이미지 삽입 (대략적인 분리)
...
post_data = {
    "title": f"[딥다이브] {TARGET_SPECIES}, 당신이 몰랐던 진짜 모습",
